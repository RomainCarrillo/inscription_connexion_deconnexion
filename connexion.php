...
// Placer les cookies
setcookie("pseudo", "", time()+(60*60*24*14), null, null, false, true);
setcookie("mdp", "", time()+(60*60*24*14), null, null, false, true);
setcookie("user_id", "", time()+(60*60*24*14), null, null, false, true);
...
<?php
    try {
        $bdd = new PDO('mysql:host=localhost;dbname=espace_membres;chartset=utf8', 'root', 'root', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    } catch(Exception $e) {
    ...
    if ((!empty($_POST['pseudo'])) AND (!empty($_POST['mdp'])) AND (isset($_POST['submit']))) {
        ...
        $req_membre = $bdd->prepare('SELECT id, mdp, pseudo FROM membres WHERE pseudo = :pseudo');
            ...
            $message_retour = "Utilisateur et/ou mot de passe incorrect.";
               ...
               if (!(password_verify($_POST['mdp'], $donnee_membre['mdp']))) {
                   $message_retour = "Utilisateur et/ou mot de passe incorrect.";
                    ...
                    $_SESSION['user_id'] = $donnee_membre['id'];
                    $_SESSION['pseudo'] = $pseudo;
                    $message_retour = "Connexion réussie!";
                    // Tester si le membre veut rester connecté
                    if (isset($_POST['connexion_auto'])) {
                       setcookie("pseudo", $donnee_membre['pseudo'], time()+(60*60*24*14), null, null, false, true);
                       setcookie("mdp", $donnee_membre['mdp'], time()+(60*60*24*14), null, null, false, true);
                       setcookie("user_id", $donnee_membre['id'], time()+(60*60*24*14), null, null, false, true);
                    } else {
                        setcookie("pseudo","", time()+(60*60*24*14), null, null, false, true);
                        setcookie("mdp", "", time()+(60*60*24*14), null, null, false, true);
                        setcookie("user_id", "", time()+(60*60*24*14), null, null, false, true);
